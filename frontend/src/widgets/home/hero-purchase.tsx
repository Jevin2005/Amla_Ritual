...
export function HeroPurchase({ slug, name, pricePaise }: HeroPurchaseProps) {
  ...
  return (
    <div className="w-full max-w-[430px]">
      <div className="flex items-baseline justify-between gap-2 border-t border-[rgba(23,63,42,0.16)] py-2 max-[680px]:py-2">
        <span className="[color:var(--forest)] [font-family:var(--font-display)] text-[1.45rem] max-[900px]:text-[1.2rem] max-[680px]:text-[1.15rem] max-[420px]:text-[1.05rem]">
          {formatCurrency(currentPricePaise, currencyCode)}
        </span>
        <small className="[color:var(--muted)] text-[0.62rem] tracking-[0.06em] max-[680px]:text-[0.55rem] max-[420px]:text-[0.5rem] max-[680px]:opacity-90">
          {source === "shopify" ? "Live price" : "Preview price"}
        </small>
      </div>
      <div className="flex items-center gap-2 max-[680px]:mt-1 max-[680px]:gap-1.5">
        <button
          className="inline-flex min-h-[52px] min-w-[min(245px,calc(100%_-_62px))] items-center justify-center gap-2.5 rounded-full border border-transparent bg-[var(--forest)] px-5 py-3 text-[0.72rem] leading-none font-bold tracking-[0.1em] text-[var(--paper)] uppercase shadow-[0_10px_26px_rgba(21,59,45,0.2)] [transition:transform_300ms_var(--ease),background-color_300ms_var(--ease),box-shadow_300ms_var(--ease)] motion-reduce:transition-none hover:bg-[var(--forest-dark)] hover:shadow-[0_14px_30px_rgba(21,59,45,0.24)] hover:[transform:translateY(-2px)] active:[transform:scale(0.98)] max-[900px]:min-h-[44px] max-[900px]:px-3.5 max-[900px]:text-[0.66rem] max-[680px]:min-h-[38px] max-[680px]:min-w-0 max-[680px]:w-auto max-[680px]:gap-1.5 max-[680px]:px-4 max-[680px]:py-2 max-[680px]:text-[0.6rem] max-[680px]:tracking-[0.07em] max-[420px]:min-h-[34px] max-[420px]:px-3.5 max-[420px]:text-[0.56rem]"
          type="button"
          onClick={() => void add()}
      ...
      <a
        className="group mt-2.5 inline-flex items-center gap-1.5 text-[0.62rem] font-bold tracking-[0.1em] text-[var(--forest)] uppercase max-[680px]:mt-2 max-[680px]:gap-1.5 max-[680px]:text-[0.58rem]"
