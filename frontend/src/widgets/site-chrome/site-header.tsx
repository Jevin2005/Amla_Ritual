...
export function SiteHeader() {
  ...
  return (
    <>
      <a
        className="fixed top-[max(12px,env(safe-area-inset-top))] left-[max(12px,env(safe-area-inset-left))] z-[9999] inline-flex min-h-11 items-center bg-[var(--paper)] px-[18px] py-[11px] text-[0.8rem] font-bold text-[var(--forest)] transition-[transform] duration-[180ms] ease-[ease] [transform:translateY(calc(-100%_-_max(24px,env(safe-area-inset-top))))] focus:[transform:translateY(0)]"
        href="#main-content"
      >
        Skip to content
      </a>
      <div className="bg-[var(--forest-dark)] text-[rgba(255,255,255,0.82)] max-[680px]:pt-[env(safe-area-inset-top)]">
        <div className="mx-auto flex min-h-8 w-full max-w-[1440px] items-center justify-between gap-8 px-[clamp(20px,3.35vw,48px)] py-1.5 text-[0.61rem] font-[650] tracking-[0.16em] uppercase max-[900px]:justify-center max-[680px]:min-h-[30px] max-[680px]:pr-[max(12px,env(safe-area-inset-right))] max-[680px]:pl-[max(12px,env(safe-area-inset-left))] max-[680px]:text-[0.6rem] max-[680px]:tracking-[0.08em]">
      ...
      <header
        className={`sticky top-0 z-[100] border-b border-[rgba(23,63,42,0.12)] bg-[rgba(247,244,232,0.92)] backdrop-blur-[16px] transition-[height,box-shadow] duration-[420ms] ease-[var(--ease)] max-[680px]:h-[60px] ${
          compact
      ...
      >
        <nav
          className="mx-auto grid h-full w-full max-w-[1440px] grid-cols-[1fr_auto_1fr] items-center px-[clamp(20px,3.35vw,48px)] max-[900px]:px-4 max-[680px]:pr-[max(16px,env(safe-area-inset-right))] max-[680px]:pl-[max(16px,env(safe-area-inset-left))]"
        ...
        >
          ...
          <button
            className="hidden h-11 w-11 shrink-0 flex-col items-center justify-center gap-1.5 bg-transparent max-[680px]:col-start-1 max-[680px]:row-start-1 max-[680px]:flex max-[680px]:justify-self-start"
            type="button"
          ...
          <Link
            className="z-10 justify-self-center px-6 py-2.5 max-[900px]:px-2 max-[680px]:col-start-2 max-[680px]:row-start-1 max-[680px]:px-0 max-[680px]:py-1"
            href="/"
            aria-label="NatureMist home"
          >
            <BrandMark />
          </Link>
          <div className="flex min-w-0 items-center justify-end gap-[clamp(20px,2.5vw,42px)] max-[1180px]:gap-4 max-[900px]:gap-2.5 max-[680px]:col-start-3 max-[680px]:row-start-1 max-[680px]:justify-self-end max-[680px]:gap-0">
            <button
            ...
            <button
              className={`gap-2 max-[680px]:px-1 max-[680px]:text-[0.58rem] ${interactiveNavClass}`}
              type="button"
              onClick={openCart}
            >
              Bag{" "}
