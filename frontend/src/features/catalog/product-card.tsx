  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-square w-full flex-none overflow-hidden bg-[var(--paper)]">
        {/* Wishlist button */}
        <button
          className={`absolute top-3 right-3 z-[3] grid size-10 place-items-center rounded-full border text-[1rem] shadow-[0_4px_16px_rgba(21,59,45,0.12)] backdrop-blur-md transition-[background,color,transform,border-color] duration-200 hover:scale-105 active:scale-95 max-[680px]:top-2 max-[680px]:right-2 max-[680px]:size-11 max-[680px]:text-[0.9rem] ${
            wished
        ...
        >
          <Image
            ...
            alt={imageAlt}
            fill
            sizes="(max-width: 420px) 100vw, (max-width: 1140px) 50vw, 33vw"
      ...
      <div className="flex flex-1 flex-col border-t border-[var(--line)] px-5 pt-4 pb-5 max-[680px]:px-3.5 max-[680px]:pt-3 max-[680px]:pb-3.5">
        ...
        <Link
          href={`/shop/${product.slug}`}
          className="mb-1 block [font-family:var(--font-display)] text-[clamp(1.45rem,1.75vw,1.95rem)] leading-[1.05] tracking-[-0.03em] text-[var(--forest)] transition-colors duration-200 hover:text-[var(--product-accent)] max-[680px]:mb-1 max-[680px]:text-[1.14rem]"
          ...
          <div className="flex items-center justify-between gap-2 border-t border-[var(--line)] pt-3 max-[680px]:pt-2">
            <div>
              <span className="flex flex-wrap items-baseline gap-x-1.5 gap-y-0.5">
                ...
                {compareAtPrice && (
                  <span
                    className="text-[0.68rem] text-[var(--muted)] line-through max-[680px]:text-[0.6rem]"
              ...
              <span
                className={`mt-0.5 block text-[0.55rem] font-bold tracking-[0.08em] uppercase max-[680px]:text-[0.58rem] ${
